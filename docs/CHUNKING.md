...
`source.md`가 정본이며 `chunks.jsonl`은 재현 가능한 파생 산출물이다. 전용 프로필이 있는 문헌은 확인 가능한 구조 경계로 청크를 만든다. 전용 프로필이 없는 문헌은 원문 행을 보존하며 길이를 기준으로 분할한다.
...
| 경악전서 | 집·권 | 논·편·변·기·음 |
| 의학입문 | 전용 구조 프로필 없음 | 원문 행을 보존하는 길이 기반 기본 분할 |

긴 단위는 1,200자에서 나누며, 매우 짧은 같은 구조의 연속 단위는 합친다. HWP 변환의 반복 머리말과 `경악전서`의 선행 목차는 본문 경계로 취급하지 않는다.

의학입문은 `paragraph-fallback-v1`로 790개 청크를 제공합니다. 원문 행 범위·해시·재생성 일치는 검증했으며 사용자 변환 QC를 통과했습니다. 권·편·조문 경계에 따른 구조 청킹은 아직 검증하지 않았습니다.
