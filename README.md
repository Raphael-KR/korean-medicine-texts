...
이 저장소의 완료 지점은 **외부 AI/RAG 시스템이 안정적으로 가져다 쓸 수 있는 지식베이스 제공**입니다.

저장소는 공개 가능한 원본, canonical Markdown 원문, 서지·권리·품질 메타데이터, 구조 기반 검색 청크, 카탈로그와 검증 기록을 제공합니다. 반면 검색 서비스나 API/UI, 임베딩 생성·호스팅, 벡터 데이터베이스, 모델 서빙, reranker 및 운영형 RAG 애플리케이션은 이 저장소의 범위가 아닙니다. 이러한 기능은 이 데이터셋을 소비하는 별도 시스템에서 구현해야 합니다.

`chunks.jsonl`은 외부 시스템이 키워드·임베딩·하이브리드 검색 등 원하는 방식으로 인덱싱할 수 있는 이식 가능한 파생 데이터입니다. 청크 제공 자체를 이 저장소의 시맨틱 검색 서비스 제공으로 해석하지 않습니다.
...
texts/
  <stable-id>/
    ...
    chunks.jsonl    # 외부 검색/RAG 시스템용 구조 기반 파생 청크
